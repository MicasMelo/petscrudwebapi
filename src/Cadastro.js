...
function App() {
  ...
  await axios
    ...
    .then(() => {
      const dadosAuxiliar = data.map((animal) =>
        animal.id === animalSelecionado.id ? animalSelecionado : animal
      );
  ...
  await axios
    .delete(baseUrl + "/" + animalSelecionado.id, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    })
    .then(() => {
      setData(data.filter(animal => animal.id !== animalSelecionado.id));
